...
AppointmentList.prototype = {
	...
	renderEle: function(data){
		...
		for (var i = 0; i < data.length; i++) {
			str += '<div class="row" data-id='+data[i].id+'><div class="col-sm-3 col-xs-3 col-md-3"><div class="innerTop">';
			...
			str += '</div></div><div class="col-sm-6 col-xs-6 col-md-6"><div class="innerTop"><p class="colorGray ">&nbsp;</p></div><div class="innerBottom">';
			str += '<p class="colorGray">&nbsp;&nbsp;&nbsp;总价:<span>'+(data[i].signMoney/10000).toFixed(0)+'</span>万元　'+(data[i].signSalesRatio*100).toFixed(0)+'%</p>';
			str += '</div></div><div class="col-sm-3 col-xs-3 col-md-3 text-right"><div class="innerTop">';
			str += '<p class="colorGray"><span>'+data[i].paymentDate+'</span></p>';
			str += '</div><div class="innerBottom">';
	...
	getData: function(propertyID,timeselete,SalesUserLeader,saleUserID,pageNum){
		...
		{
			$.ajax({
			    url: getRoothPath+'/saleusercontroller/payMentSaleListApp.json',
			    data: { 
			    ...
			    success:function(data){
			    	if (JSON.stringify(data) !== "{}") 
			    	{
			    		...
			    		$my.paymentSum.innerHTML = (data.paymentSum/10000).toFixed(0);
	...
	turnDetail: function(){ //详情页面点击
		...
		window.location.href = "huiKuan_detail.html?detailed="+detailed;
	}
}
...
$(function() {
	window.$my={
		...
		paymentSum: document.querySelector("#paymentSum"),
		flag: false //滑动标志位
	}
	...
	appointmentList.renderSalesUserList(); //获取组id
