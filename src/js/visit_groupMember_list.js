...
AppointmentList.prototype = {
	...
	renderEle: function(data){
		...
		for (var i = 0; i < data.length; i++) {
			str += "<div class='row' data-id="+data[i].id+"><div class='col-sm-8 col-xs-8 col-md-8'><div class='innerTop'>";
			str += "<p class='bigFont nowrap'><span>"+data[i].customerName+"</span>　<span>"+data[i].customerTel+"</span></p>";
			str += "</div><div class='innerBottom'>";
			str += "<p class='colorGray'>客户意向:<span>"+data[i].houseName+"</span>　　<span>"+data[i].num+"</span>访</p>";
			str += "</div></div><div class='col-sm-4 col-xs-4 col-md-4 text-right'><div class='innerTop'>";
			str += "<p class='colorGray'><span>"+data[i].lookTime+"</span></p>";
			str += "</div><div class='innerBottom'>";
	...
	getData: function(propertyID,timeselete,SalesUserLeader,saleUserID,pageNum){
		...
		{
			$.ajax({
			    url: getRoothPath+'/saleusercontroller/lookSaleListApp.json',
			    async: false, //同步
			    data: { 
	...
	turnDetail: function(){ //详情页面点击
		...
		window.location.href = "visit_detail.html?detailed="+detailed;
	}
}
