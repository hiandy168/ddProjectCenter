var ReportDetail = (function(window) {
	...
	ReportDetail.fn = ReportDetail.prototype = {
		...
		init: function() {
			...
			this.signMoney = ""; //签约总价
			this.dealArea = ""; //签约面积
			this.purchaseWay = ""; //购房方式
			this.salePrice = ""; //首付金额
			this.groupName = ""; //团购方案
			this.groupShouldpay = ""; //应收团购
			this.groupActualpay = ""; //实收团购
			this.remark = ""; //备注信息
			...
			this.config = {
				...
				purchaseWay: document.querySelector("#purchaseWay"),
				...
				groupActualpay: document.querySelector("#groupActualpay"),
		...
		bindDom_init: function(){ //绑定DOM
			...
			this.config.purchaseWay.innerHTML = this.purchaseWay;
			...
			this.config.groupActualpay.innerHTML = this.groupActualpay;
				    ...
				    success:function(data){
				    	if (JSON.stringify(data) !== "{}") 
				    	{
				    		...
				    		if (JSON.stringify(json) !== "{}") 
				    		{
				    			...
				    			reportDetail.purchaseWay = json.purchaseWay; 
				    			...
				    			reportDetail.groupActualpay = (json.groupActualpay/10000).toFixed(0);
				    			...
				    			reportDetail.renderPaydata_init(data.paydata);
				    		} else
				    		{
		...
		renderPaydata_init: function(data){
			if (data.length) {
				var str = "";
				var paydata = document.getElementById("paydata");

				for (var i = 0,len = data.length; i < len; i++) {
					if (data[i].paymentDate.toString().trim() === "") {
						data[i].paymentDate = "--";
					};

					if (data[i].paymentModle.toString().trim() === "") {
						data[i].paymentModle = "--";
					};

					if (data[i].payment.toString().trim() === "") {
						data[i].payment = "--";
					};

					if (data[i].saleName.toString().trim() === "") {
						data[i].saleName = "--";
					};

					str += '<tr>';
					str += '<td><span>'+data[i].paymentDate+'</span></td>';
					str += '<td><span>'+data[i].paymentModle+'</span></td>';
					str += '<td><span>'+(data[i].payment/10000).toFixed(0)+'</span>万元</td>';
					str += '<td><kbd class="bgcGray"><span>案</span>:<span>'+data[i].saleName+'</span></kbd></td>';
					str += '<tr>';
				};

				paydata.innerHTML = str;
			} else{
				$my.messageInfo.html("暂无回款信息").fadeIn("fast").delay("1000").fadeOut("slow");
				return false;
			};
		}
	}
