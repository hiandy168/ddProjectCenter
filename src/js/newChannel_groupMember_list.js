...
AppointmentList.prototype = {
	...
	renderEle: function(data){
		...
		for (var i = 0; i < data.length; i++) {
			...
			str += '<p class="nowrap bigFont" data-toggle="tooltip" data-placement="top" title="">'+data[i].agencyName+'</p>';
			str += '</div><div class="innerBottom">';
			str += '<p class="colorGray"><span>'+data[i].LinkName+'</span>　　<span>'+data[i].agencyTel+'</span></p>';
			str += '</div></div><div class="col-sm-4 col-xs-4 col-md-4 text-right"><div class="innerTop">';
			str += '<p class="colorGray"><span>'+data[i].number+'</span>人</p>';
			str += '</div><div class="innerBottom">';
	...
	getData: function(propertyID,timeselete,PlaceUserLeader,placeUserID,pageNum){
		...
		{
			$.ajax({
			    url: getRoothPath+'/placeusercontroller/addAgencyListApp.json',
			    data: { 
