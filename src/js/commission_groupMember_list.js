...
AppointmentList.prototype = {
	...
	renderEle: function(data){
		...
		for (var i = 0; i < data.length; i++) {
			str += '<div class="row" data-id='+data[i].id+'><div class="col-sm-6 col-xs-6 col-md-6"><div class="innerTop">';
			str += '<p class="bigFont"><span>'+data[i].houseName+'</span></p>';
			str += '</div><div class="innerBottom">';
			str += '<p class="colorGray nowrap">'+data[i].agencyName+'</p>';
			str += '</div></div><div class="col-sm-3 col-xs-3 col-md-3"><div class="innerTop"><p class="colorGray bigFont">&nbsp;</p></div><div class="innerBottom">';
			str += '<p class="colorGray text-center">佣:<span>'+(data[i].commissionAmount/10000).toFixed(0)+'</span>万元</p>';
			str += '</div></div><div class="col-sm-3 col-xs-3 col-md-3 text-right"><div class="innerTop">';
			str += '<p class="colorGreen"><span>已发</span></p>';
			str += '</div><div class="innerBottom">';
	...
	getData: function(propertyID,timeselete,PlaceUserLeader,placeUserID,pageNum){
		...
		{
			$.ajax({
			    url: getRoothPath+'/placeusercontroller/commissionPlaceListApp.json',
			    data: { 
			    ...
			    success:function(data){
			    	if (JSON.stringify(data) !== "{}") 
			    	{
			    		...
			    		$my.sumCommssion.innerHTML = (data.sumCommssion/10000).toFixed(0);
	...
	turnDetail: function(){ //详情页面点击
		...
		window.location.href = "commission_detail.html?detailed="+detailed;
	}
}
...
$(function() {
	window.$my={
		...
		sumCommssion: document.querySelector("#sumCommssion"),
