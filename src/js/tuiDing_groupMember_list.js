...
AppointmentList.prototype = {
	...
	renderEle: function(data){
		...
		for (var i = 0; i < data.length; i++) {
			...
			str += '<p class="colorGray"><span>'+data[i].houseName+'</span>　　　　总价:<span>'+(data[i].signedMoney/10000).toFixed(0)+'</span>万</p>';
			str += '</div></div><div class="col-sm-4 col-xs-4 col-md-4 text-right"><div class="innerTop">';
			str += '<p class="colorGray"><span>'+data[i].tdTime+'</span></p>';
			str += '</div><div class="innerBottom">';
	...
	getData: function(propertyID,timeselete,SalesUserLeader,saleUserID,pageNum){
		...
		{
			$.ajax({
			    url: getRoothPath+'/saleusercontroller/unsubscribeSaleListApp.json',
			    data: { 
	...
	turnDetail: function(){ //详情页面点击
		...
		window.location.href = "tuiDing_detail.html?detailed="+detailed;
	}
}
